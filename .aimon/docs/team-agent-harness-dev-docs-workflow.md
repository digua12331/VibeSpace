...
主理人只负责方向和验收；主 Claude 负责拆解、派工、执行、验证和留痕；Harness 负责隔离、日志、后台任务和记忆。

**主理人是非程序员**。能拿给他确认的只有两类：（1）大方向（这次任务到底要解决什么、什么样算解决了）；（2）有用户感知差异的分叉（A 路径和 B 路径会让用户最终看得见的东西不一样：界面不同 / 数据会不会动到 / 能否撤销 / 操作步骤变没变）。**纯内部实现**的分叉（用 A 函数还是 B 函数、放 utils 还是单独文件、要不要抽 helper），AI 自决，不要打扰。

默认行为：

- Plan 阶段默认走 Claude / Gemini / Codex 三模型会审，Codex 综合定稿，主 Claude 白话化落盘；只停一次等主理人确认。**这是整条流程里唯一一次**。
- Context / Tasks / 执行阶段不等确认。Context 写完直接进 Tasks，不要发“context 写好了请确认”这种消息。
- 技术细节自行决定。
- 所有执行通过文件、日志、任务状态留痕。
- 只有方向、数据安全、验收不可行、连续失败时才打断主理人。

## 0.1 跟主理人说话的规矩（硬性，贯穿全流程）
...
写完一段面向主理人的话，回头检查一遍：里面还有没有裸术语？有没有“做了什么技术动作”而不是“用户能看到什么变化”？有就改。
...
主理人主要靠这段决定要不要点头。**写得不像人话就是不合格**——专业术语裸奔、说技术动作而不说用户感知变化、列 A 还是 B 给主理人选，都算不合格，要重写。

主理人只确认这一层。不要把技术选择拿给主理人拍板。
...
主理人确认 plan 后，直接写 `context.md`，**不要发“context 写好了请确认”这种消息**——这一段是给 AI 自己看的，主理人不审。写完直接进 Tasks 阶段。
...
主 Claude 如果不开 worktree，要能说明风险很低；如果开 worktree，交付摘要里要提醒主理人当前改动在隔离分支里。
...
| 主理人偏好 | `dev/memory/manual.md` | 手动追加 |
...
交付摘要 ≤10 行，**写给主理人看，不是工程笔记**。**第一行必须是验收指引**——白话告诉主理人“现在去哪里点哪里能看到效果”：
...
术语括号解释，符合 0.1 节“跟主理人说话的规矩”。**没有这段摘要不算交付完成**；第一行不是验收指引也不算合格。
