...
**默认 OFF**。原来的 toggle 默认 ON 是因为它就在表单里能马上看到、能马上关；迁移后第一次建项目就强制写 CLAUDE.md 会让"轻量试用项目"也被侵入。建后用户可随时进设置抽屉打开。**这是与现状的行为变更，需大哥确认**。
...
1. **`F:\VibeSpace` 硬编码**：临时方案。`DEFAULT_ROOT` 是常量，未来可改环境变量 `AIMON_DEFAULT_ROOT` 一行迁移。**假设大哥接受当前 Windows + 固定盘符场景。**
...
5. **行为变更需大哥确认**：建项目默认不再自动应用 Dev Docs（原默认 ON）；关 Dev Docs 弹确认；Harness 移入高级折叠区。
