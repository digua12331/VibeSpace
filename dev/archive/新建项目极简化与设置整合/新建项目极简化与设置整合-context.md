...
- L69 `mode` state 类型扩为 `'workflow' | 'permissions' | 'buttons'`，初值 `'workflow'`（**待主理人确认默认 tab，见决策 7**）。
...
### 决策 7：`mode` 初值 `'workflow'`（**待主理人在开工前拍板**）

打开设置抽屉默认落"工作流" tab。如果主理人偏好默认仍是"权限"（多数人用抽屉是为了改权限不是为了切 Dev Docs），改回 `'permissions'` 即可。
