...
"施工边界（SessionScope）"是给 session 配 readwrite/readonly glob、由后端 `/api/hooks/claude` 在 PreToolUse 阶段拦 Edit/Write/NotebookEdit/MultiEdit 的功能。主理人在使用中确认它的实战性价比不高（要预先列 glob、被拦后打断节奏、跟 worktree 隔离 + 看 diff 这条主路相比冗余），决定整个拆掉。
