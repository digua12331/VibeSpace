...
大哥是 vibe coder、不看代码，现有 CLAUDE.md 三段式工作流只能"提示词劝说"AI 别动核心模块。一旦 AI 为了接通现有系统"顺手重构"了 `core/` 或 `packages/server` 等关键目录，大哥从 UI 上察觉不到，等真实后果爆出来可能是几天后。
...
在起 AI 会话时允许大哥勾选本会话的施工边界（两个 glob 列表：可写 / 只读），写入 SQLite。Claude 每次 `PreToolUse` 触发时，aimon-hook.mjs 调本地 API 查当前 session 的 scope，按 glob 匹配决定放行或阻断。
...
- **Hook API 超时或后端不可达**：**fail-open**（放行并在 hook stdout 的 reason 写一条 warning，让 AI 的反馈里能看到"scope 暂时失效"）。阻断优先让会话能进行，不要让大哥陷入"AI 卡死 + 我不知道为什么"的死境。
