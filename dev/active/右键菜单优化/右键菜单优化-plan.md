...
- 假设 3：未提交数的口径是 "staged + unstaged + untracked"（ChangesList / VS Code 口径）。如果用户要的是只算"已跟踪的 uncommitted（不含 untracked）"，请在确认时指出。
- **两种解读请用户选一**：
  - **方案 A（推荐）**：菜单锚定点击的**项目行右边**（横向贴右，纵向对齐行顶）。好处：每个项目独立、直观；切换项目时菜单随项目移动。
  - **方案 B**：所有项目共用一个固定点（例如屏幕左侧列的右边、顶部偏移某固定 y）。好处：完全固定；坏处：看不出是对哪个项目操作的。
  - 默认按 A 实施，如要 B 请在确认时说。
