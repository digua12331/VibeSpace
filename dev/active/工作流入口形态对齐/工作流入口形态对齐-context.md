...
- `isHarnessApplied()` 探测点：`existsSync(join(projectPath, ".aimon", "skills"))`（L135）。**uninstall 只删文件不删目录，此探测点卸载后仍返 true**——会导致下次 mount 状态回弹为"已应用"。**见决策 2 / 待大哥拍板**。
...
### 决策 2：`isHarnessApplied` 探测点回弹问题（**待大哥拍板**）
...
context 倾向 **本次修**（rmdir 自带空目录保护，复杂度低，避免大哥卸载后困惑）。
