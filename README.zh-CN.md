...
- **内部弹窗统一**：不用原生 `alert` / `confirm`，全是页面内 modal，
  ESC / Enter / 点遮罩都行为一致。
- **技能管理面板（🧩）**：浏览器里管理 *Anthropic 标准* skill 目录布局——
  `.claude/skills/`、`.codex/skills/`、`.opencode/skill[s]/`——也就是底层
  AI CLI 自己读取的那套。按 agent 切换：扫描项目 + 全局、把全局 skill 装
  进项目（复制或 symlink，权限不足自动回退到复制）、卸载、从任意自定义
  路径添加。**注意**：这跟 `.aimon/skills/<name>.md`（单文件 + 触发词，
  由 VibeSpace SessionStart hook 注入提示词片段）是**两套互不干扰**的机
  制；本面板管的是 CLI 自身的 skill 系统。
- **技能市场（🛒，嵌在 🧩 面板里）**：同一个面板的第二种模式，能一次搜
  GitHub `topic:skill` + skills.sh，点一下 `git clone --depth 1` 到本地库
  （默认 `~/SkillManager/`，路径可改，存在 `~/.vibespace/skill-market.json`），
  下载完直接用上一条的"装到本项目"按钮安装。安全加固：repoUrl 白名单
  正则、GitHub 搜索 60 秒内存缓存、单并发下载 + 60 秒超时、50 MB / 5000
  文件大小上限、`cpSync` 走 `dereference: true` 防 symlink 越狱、临时目录
  finally 清理。**只有点搜索或下载时才联网**，面板不会主动连外网。
...
```
   ...
   │   ├── comments           — 文件评论 CRUD
   │   ├── issues             — dev/issues.md 读取器
   │   ├── memory             — dev/memory/auto.md + manual.md 读取器
   │   ├── usage              — Claude 使用统计
   │   ├── skill-catalog      — 扫描 / 安装 / 卸载 .claude|.codex|.opencode/skills/
   │   ├── skill-market       — GitHub + skills.sh 搜索 / git clone 下载 / 本地库
   │   └── health
   ...
   ├── CommentsService        — 文件评论管理
   ├── IssuesService          — dev/issues.md 读取器
   ├── MemoryService          — dev/memory 管理
   ├── UsageService           — Claude 使用跟踪
   ├── SkillCatalogService    — 文件夹形式的 skill（Claude / Codex / OpenCode）
   ├── SkillMarketService     — 搜索 / 下载 / 本地库 + 安全：regex 白名单 / 大小上限 / dereference cp
...
| GET  | `/api/projects/:id/comments?path=` | 列出文件评论 |
...
│   │       ├── comments-service.ts 文件评论管理
│   │       ├── issues-service.ts   dev/issues.md 读取器
│   │       ├── memory-service.ts   dev/memory 管理
│   │       ├── usage-service.ts    Claude 使用跟踪
│   │       └── routes/
