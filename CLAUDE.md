...
- **大任务自拆并行**（≥ 5 个独立可并行子任务时启用）：如果任务能拆成 N 个相对独立的子任务（write_files 基本不重叠 + 有清晰依赖关系），AI 自己在 plan.md 最末尾追加 `## 自拆与依赖` 段，里面写一个 ```` ```json ```` 代码块描述子任务结构（schema 见 `.aimon/templates/subtasks-syntax.example.md`）。前端 Dev Docs 任务行下会自动显示「子任务」面板，大哥可以"一键派工"让每个子任务跑在独立 worktree、"approve 全部"按拓扑序 merge。**不能拆的任务不写这段**——UI 自动隐藏入口。
...
- **tasks.md 是状态的唯一真源；执行期不要逐步回填 tasks.json 的 `status`**。每完成一步只改 tasks.md 的 `- [ ]` → `- [x]`，看板进度由后端直接数 tasks.md 的复选框得出（见 `packages/server/src/docs-service.ts::summarizeTask`），不需要同步 json。tasks.json 在建任务时写一次（步骤结构 + `read_files`/`write_files` 白名单），执行期不用再动它的 `status` 字段——这样消除了"两份文件状态对不上"的老毛病。
...
## 任务完结后的代码学习指引（硬性，大哥专属）

大哥在补代码能力（半年目标：从"全靠 AI 看不懂"到"读得懂 / 改得动 / 会 debug / 判断对错"，语言优先级 Python > C# > Lua）。所以**凡是产生了代码改动的任务，交付 handoff 摘要之后，必须再附一段「## 学习指引」**，把这次写的代码顺手教给他。这是硬性交付物，不是可选项。

- **触发条件**：本次任务**改动了代码**（源码 / 脚本 / 配置逻辑）。纯文案、纯文档、纯归档、纯 md 改动**跳过**（没有代码可教）。
- **指引内容**（照 `docs/求职/边开发边学-代码导师提示词.md` 里那套来，那份是单一真源，规则更新只改那里）：
  1. **白话讲清本次代码核心**：这次改动的关键代码"对数据/用户做了什么"，挑有信息量的几行逐行白话解释。
  2. **翻译黑话**：本次出现的、他可能不懂的术语（编程黑话 + 项目/游戏黑话）用括号配白话。
  3. **留一个动手钩子**：问他一个检查问题，或指一处让他自己改着玩的小地方——让他动手，不只是听。
  4. **攒黑话本**：把这次教的新术语，按格式追加到 `docs/求职/学习记录/黑话本.md`（已记过的不重复）。
  5. **偶尔联系面试**：碰到技术面常考点（空值处理 / 序列化 / 性能 / 兼容旧数据 / 数据结构选型），顺口提一句"这个面试会问"。
- **出 bug 时**：别默默修好，先带他走一遍排查（看哪条报错→怀疑哪→怎么缩小范围→再修），修完告诉他"以后这类问题你自己怎么查"。
- **节奏**：别把每次都搞成讲座——简单改动一两句带过，有信息量的、他没见过的、面试可能问的才展开。目标是他每次都学到一两个能记住的东西。
